...
impl FromStr for Token {
	...
	fn from_str(token_str: &str) -> std::result::Result<Self, Self::Err> {
		...
		if splits.len() != 3 {
			return Err(Error::TokenInvalidFomat);
		}
		...
		Ok(Self {
			ident: b64u_decode(ident_b64u)
				.map_err(|_| Error::TokenCannontDecodeIdent)?,

			exp: b64u_decode(exp_b64u).map_err(|_| Error::TokenCannontDecodeExp)?,
...
mod tests {
	...
	fn test_token_display_ok() -> Result<()> {
		// -- Fixtures
		let fx_token_str =
			"ZngtaWRlbnQtMDE.MjAyNC0xMC0wMVQwODozMDowMFo.some-sign-b64u-encoded";
		let fx_token = Token {
			ident: "fx-ident-01".to_string(),
			exp: "2024-10-01T08:30:00Z".to_string(),
